...
def render_md_block(text: str, from_path: str = "") -> str:
    ...
    while i < len(lines):
        ...
        # Mermaid diagrams and other preformatted blocks ride through this
        # as `<pre><code class="language-{lang}">…</code></pre>` so browser-
        # side libraries (e.g. mermaid.js) can pick them up by class.
        if stripped.startswith("```"):
            ...
            out.append(f'<pre><code{lang_attr}>{html.escape(code_text)}</code></pre>')
            continue
...
def _render_moc_extras(stage_n: int, repo_root: Path) -> str:
    ...
    en_candidates = [
        repo_root / "Quests" / f"Stage {stage_n}.en.md",
        repo_root / "Quests" / f"Stage {stage_n}" / f"Stage {stage_n}.en.md",
    ]
    ...
    pt_body = _strip_fm(moc_text)
    en_body = _strip_fm(en_text) if en_text else ""
    ...
    pt_md = _filter(pt_body)
    if not pt_md and not en_body:
        return ""

    # Render PT and (optional) EN through the same from_path so wiki-link
    # resolution is consistent. When EN isn't provided, fall back to PT
    # text in both spans so toggling language never shows an empty block.
    pt_html = render_md_block(pt_md, from_path=str(moc_path))
    en_html = render_md_block(_filter(en_body), from_path=str(en_path)) if en_body else pt_html

    return (
        '<section class="moc-extras">\n'
        + render_bilingual_raw(en_html, pt_html)
        + "\n</section>"
    )
  ...
  <div class="summary-bar-actions">
    ...
    for idx, (qtype, fname) in enumerate(flow_order):
        ...
        if has_dividers and not is_last:
            ...
            # NOTE: _split_objectives_at_dividers returns
            #   [(pt_chunk, en_chunk), ...]
            # i.e. a list of (pt, en) pairs — DO NOT destructure as
            # `(pt_chunks, en_chunks)`, that transposes them and Part 2
            # gets Part 2's PT bundled as Part 1's EN (visual dup of
            # objectives + ID collisions).
            chunks = _split_objectives_at_dividers(q_pt, q_en)
            if len(chunks) == 2:
                (pt1, en1), (pt2, en2) = chunks
                # Part 1 only at the MOC position
                q_pt_p1 = replace(q_pt, objectives=pt1)
                q_en_p1 = replace(q_en, objectives=en1)
                ...
                # Part 2 deferred; track_id_offset = len(pt1) so the
                # first Part 2 checkbox continues numbering right
                # after the last Part 1 checkbox.
                q_pt_p2 = replace(q_pt, objectives=pt2)
                q_en_p2 = replace(q_en, objectives=en2)
                q_pt_p2.title = f"{q_pt.title} — Parte 2"
                q_en_p2.title = f"{q_en.title} — Part 2"
                deferred_part2s.append((q_en_p2, q_pt_p2, len(pt1)))
                continue
