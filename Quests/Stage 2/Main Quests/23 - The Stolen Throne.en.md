---
quest: The Stolen Throne
type: Main Quest
stage: 2
location: "[[Locations/Vernworth]]"
quest_giver: Brant
prerequisites:
  - "[[21 - The Caged Magistrate]]"
related:
  - "[[Locations/Vernworth]]"
  - "[[12 - Oxcart Courier]]"
  - "[[20 - Disa's Plot]]"
tags:
  - quest
  - stage-2
  - main-quest
sources_verified:
  - fextralife
needs_verification: true
---

# The Stolen Throne

## Summary

Main quest of political climax. Brant discovers the throne has been stolen. The Arisen needs to find the thief and recover the throne — or expose the larger conspiracy.

## How to Unlock

Complete [[21 - The Caged Magistrate]] and report to Brant.

## Quest Giver

- **NPC**: Brant
- **Location**: [[Locations/Vernworth]]

## Prerequisites

- [[21 - The Caged Magistrate]] *(completed)*

## Walkthrough

1. After completing [[21 - The Caged Magistrate]], find Brant at The Stardrop Inn — he tells you the throne was stolen during the events of the previous quest.
2. Brant designates a guard to accompany you on the investigation; accept the escort.
3. Go to the **Royal Quarter / Vernworth castle** with the guard and investigate the throne room — collect clues (footprints, overturned furniture, marks of forced entry).
4. Cross Vernworth interrogating witnesses (Sven in the Merchant Quarter usually saw something; consult Kendrick and other nobles listed in `[[Quests/Stage 2.md#🎯 NPCs notáveis]]`).
5. Follow the clues until you identify the thief. The exact unfolding varies based on outcomes of [[20 - Disa's Plot]] and [[12 - Oxcart Courier]] (see the branch documented in those quests).
6. Confront / recover the throne and report to Brant — this quest closes the intermediate political triad before [[24 - An Unsettling Encounter]].

> [!warning] Branch pending verification
> The **identified thief** + exact steps depend on the wiki source (Fextralife / Fandom 404 in this session — fill in `## Rewards` and the thief's identifier when the source loads).

## Objectives

- [ ] Talk to Brant about the throne
- [ ] Investigate the crime scene
- [ ] Collect clues
- [ ] Identify the thief
- [ ] Recover the throne

## Rewards

To be confirmed in the multi-source research phase (Fextralife / IGN / Fandom). Wiki URLs returned 404/402 in this session — retry before EN translation.

## Important Notes

> [!warning] Narrative consequences
> Choices in [[20 - Disa's Plot]] and [[12 - Oxcart Courier]] affect how this quest unfolds.

> [!info] Brant chain
> This is the conclusion of the Brant / Disa chain started in [[15 - Seat of the Sovran]].

## Related

- [[Locations/Vernworth]]
- [[12 - Oxcart Courier]]
- [[20 - Disa's Plot]]

## Sources

- Fextralife Wiki DD2

---

#dragon's dogma #quest #stage-2 #main-quest